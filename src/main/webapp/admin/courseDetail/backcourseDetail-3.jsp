			...
			<thead>
			  ...
			  <c:forEach var="x" items="${books }">
			    <tr class="text-c">
					...
					<td class="f-14 td-manage"><a style="text-decoration:none" class="ml-5" onClick="article_edit('内容修改','/DearHouAimin/admin/courseDetail/editcourseDetail-3.jsp','${x.id}')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a> <a style="text-decoration:none" class="ml-5" onClick="article_del(this,'${x.id}')" href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
				</tr>
			  </c:forEach>
			</table>
	</div>
...
function article_edit(title,url,id,w,h){
    $.ajax({
			type: 'GET',
			url: '/DearHouAimin/textServlet?method=beforeUpdate&id='+id,
			dataType: 'json',
...
function article_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'GET',
			url: '/DearHouAimin/textServlet?method=del&id='+id,
			dataType: 'json',
			success: function(data){
			    if (data.result == 1) {//请求成功
